...
class LMDB:
    ...
    def read_single(self, key):
        ...
        with env.begin() as transaction:
            ...
            label = datum.label
            if datum.data:
                image = numpy.fromstring(datum.data, dtype=numpy.uint8).reshape(
                    datum.channels, datum.height, datum.width).transpose(1, 2, 0)
            else:
                image = numpy.array(datum.float_data).astype(numpy.float).reshape(
                    datum.channels, datum.height, datum.width).transpose(1, 2, 0)
    ...
    def write(self, images, labels=[]):
        ...
        keys = []
        env = lmdb.open(self._lmdb_path, map_size=max(
            1099511627776, len(images)*images[0].nbytes))
        ...
        with env.begin(write=True) as transaction:
            for i in range(len(images)):
                ...
                assert version_compare(
                    numpy.version.version, '1.9') < 0, "installed numpy is 1.9 or higher, change .tostring() to .tobytes()"
...
lmdb_dir = '/home/alex/caffe/data/plates/lmdb/plates_trainval_lmdb'
# lmdb_dir = '/home/alex/Downloads/bla'
# lmdb_dir = '/home/alex/caffe/data/VOCdevkit/VOC0712/lmdb/VOC0712_test_lmdb'
# db = LMDB(lmdb_dir)

lmdb_env = lmdb.open(lmdb_dir, map_size=int(1e9))
...
for key, value in lmdb_cursor:
  ...
  grps = annotated_datum.annotation_group
  print(grps)
  type = annotated_datum.type
  for grp in grps:
    xmin = grp.annotation[0].bbox.xmin * datum.width
